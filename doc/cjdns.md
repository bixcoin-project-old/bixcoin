...
redundancy and robustness for both the Bixcoin network and individual nodes.
...
CJDNS peers than Tor or IP ones. You can use `bixcoin-cli -addrinfo` to see the
...
To see which CJDNS peers your node is connected to, use `bixcoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `bixcoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `bixcoin-cli help getnodeaddresses` for details.
