...
It is possible to specify `bixcoind` arguments to the `fuzz` executable.
...
Core without having to write any custom fuzzing harness. The `bixcoind` server
...
$ honggfuzz/honggfuzz --exit_upon_crash --quiet --timeout 4 -n 1 -Q \
      -E HFND_TCP_PORT=18444 -f inputs/ -- \
          src/bixcoind -regtest -discover=0 -dns=0 -dnsseed=0 -listenonion=0 \
