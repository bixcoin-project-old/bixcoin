...
The headless daemon `bixcoind` has the JSON-RPC API enabled by default, the GUI
`bixcoin-qt` has it disabled by default. This can be changed with the `-server`
...
This is the endpoint used by bixcoin-cli when a `-rpcwallet=` parameter is passed in.
...
bixcoin-cli createwallet mywallet false false "" false false true

# "params": {"wallet_name": "mywallet", "load_on_startup": true}
bixcoin-cli -named createwallet wallet_name=mywallet load_on_startup=true

# "params": {"args": ["mywallet"], "load_on_startup": true}
bixcoin-cli -named createwallet mywallet load_on_startup=true
```
...
- **Securing remote network access:** You may optionally allow other
  ...
  `bixcoind -help` for more information about these settings and other
