...
```
bixcoind -i2psam=127.0.0.1:7656
```
...
debug log about your I2P configuration and connections. Run `bixcoin-cli help
...
`bixcoin-cli -addrinfo` to see the number of I2P addresses known to your node.
...
To see which I2P peers your node is connected to, use `bixcoin-cli -netinfo 4`
or the `getpeerinfo` RPC (e.g. `bixcoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of I2P peers known to your node; run `bixcoin-cli help getnodeaddresses` for details.
