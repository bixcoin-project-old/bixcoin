...
- All JSON-RPC methods accept a new [named
  ...
  python test framework and `bixcoin-cli` tool both take advantage of this, so
  for example:

```sh
bixcoin-cli -named createwallet wallet_name=mywallet load_on_startup=1
```

Can now be shortened to:

```sh
bixcoin-cli -named createwallet mywallet load_on_startup=1
```
...
- In the `createwallet`, `loadwallet`, `unloadwallet`, and `restorewallet` RPCs, the
  ...
  deprecation period by launching bixcoind with the configuration option
