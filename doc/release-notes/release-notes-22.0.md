...
- When creating a hex-encoded bitcoin transaction using the `bixcoin-tx` utility
...
- Passing an invalid `-rpcauth` argument now cause bixcoind to fail to start.  (#20461)
...
- A new `-rpcwaittimeout` argument to `bixcoin-cli` sets the timeout
  in seconds to use with `-rpcwait`. If the timeout expires,
  `bixcoin-cli` will report a failure. (#21056)
...
- bitcoin/bitcoin#21029 bixcoin-cli: Correct docs (no "generatenewaddress" exists) (luke-jr)
...
### RPC and other APIs
- bitcoin/bitcoin#18335, bitcoin/bitcoin#21484 cli: Print useful error if bixcoind rpc work queue exceeded (LarryRuane)
...
- bitcoin/bitcoin#20549 Support make src/bixcoin-node and src/bixcoin-gui (promag)
...
- bitcoin/bitcoin#20939 fix `RELOC_SECTION` security check for bixcoin-util (fanquake)
...
- bitcoin/bitcoin#22230 Fix MSVC linker /SubSystem option for bixcoin-qt.exe (hebasto)
...
- bitcoin/bitcoin#22270 Add bixcoin-util tests (+refactors) (MarcoFalke)
...
- bitcoin/bitcoin#20715 util: Add argsmanager::getcommand() and use it in bixcoin-wallet (MarcoFalke)
...
- bitcoin/bitcoin#20884 script: Improve robustness of bixcoind.service on startup (hebasto)
...
- bitcoin/bitcoin#21007 bixcoind: Add -daemonwait option to wait for initialization (laanwj)
...
- bitcoin/bitcoin#21695 contrib: Remove no longer used contrib/bixcoin-qt.pro (hebasto)
...
- bitcoin/bitcoin#20913 Add manual page generation for bixcoin-util (laanwj)
...
- bitcoin/bitcoin#21384 doc: add signet to bixcoin.conf documentation (jonatack)
...
- bitcoin/bitcoin#21709 Update reduce-memory.md and bixcoin.conf -maxconnections info (jonatack)
...
- bitcoin/bitcoin#21818 Fixup -coinstatsindex help, update bixcoin.conf and files.md (jonatack)
...
- bitcoin/bitcoin#22080 add maxuploadtarget to bixcoin.conf example (jarolrod)
...
- bitcoin/bitcoin#22393 Added info to bixcoin.conf doc (bliotti)
