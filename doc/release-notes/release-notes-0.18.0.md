...
`/Applications/Bixcoin-Qt` (on Mac) or `bixcoind`/`bixcoin-qt` (on
Linux).
...
`contrib/init/bixcoind.service`, it has been changed to use
`/var/lib/bixcoind` as the data directory instead of
`~bitcoin/.bitcoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/bixcoind` will
exist has enough space (check using `df -h /var/lib/bixcoind`), and
...
The systemd init file (`contrib/init/bixcoind.service`) has been changed
to use `/var/lib/bixcoind` as the data directory instead of
...
- A new [document](https://github.com/bitcoin/bitcoin/blob/master/doc/bitcoin-conf.md)
  about the `bixcoin.conf` file describes how to use it to configure
  Bitcoin Core.
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Bixcoin-Qt from being built with support for the BIP70 payment
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart bixcoind with the `-deprecatedrpc=generate`
...
- A new `bixcoin-wallet` tool is now distributed alongside Bitcoin
...
- #13926 [Tools] bixcoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like bixcoind (MarcoFalke)
...
- #14521 qt, docs: Fix `bixcoin-qt -version` output formatting (hebasto)
...
- #15154 Configure: bixcoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/bixcoin-wallet (Sjors)
...
- #13953 Fix deprecation in bixcoin-util-test.py (isghe)
...
- #12246 Bugfix: Only run bixcoin-tx tests when bixcoin-tx is enabled (luke-jr)
...
- #14985 Remove `thread_local` from `test_bixcoin` (MarcoFalke)
...
- #14186 bixcoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_bixcoin.conf` message on startup if conf file exists (leishman)
...
- #15354 Add missing `bixcoin-wallet` tool manpages (MarcoFalke)
