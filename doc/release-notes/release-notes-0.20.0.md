...
- A new `-asmap` configuration option has been added to diversify the
  ...
  `bixcoind help` for more information.  This option is experimental and
...
- The `bixcoin-cli` utility used with the `-getinfo` parameter now
...
- The `bixcoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- Command line options prefixed with main/test/regtest network names like
  `-main.port=9424` `-test.server=1` previously were allowed but ignored. Now
...
- #17741 Included `test_bixcoin-qt` in msvc build (sipsorcery)
...
- #18702 Fix ASLR for bixcoin-cli on Windows (fanquake)
...
- #17330 Add `shrinkdebugfile=0` to regtest bixcoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_bixcoin` build of `*.raw` (MarcoFalke)
...
- #17119 Fix broken bixcoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from bixcoin-qt help message and manpage (laanwj)
...
- #17648 Rename wallet-tool references to bixcoin-wallet (hel-o)
...
- #18341 Replace remaining literal BIX with `CURRENCY_UNIT` (domob1812)
...
- #16115 On bixcoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from bixcoin-cli -getinfo (malevolent)
