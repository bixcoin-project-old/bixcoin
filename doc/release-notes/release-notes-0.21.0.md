...
- A new `bixcoin-cli -netinfo` command provides a network peer connections
...
- A new `bixcoin-cli -generate` command, equivalent to RPC `generatenewaddress`
  followed by `generatetoaddress`, can generate blocks for command line testing
  purposes. This is a client-side version of the former `generate` RPC. See the
  help for details. (#19133)

- The `bixcoin-cli -getinfo` command now displays the wallet name and balance for
  each of the loaded wallets when more than one is loaded (e.g. in multiwallet
  mode) and a wallet is not specified with `-rpcwallet`. (#18594)

- The `connections` field of `bixcoin-cli -getinfo` is now expanded to return a JSON
...
- The `-salvagewallet` startup option has been removed. A new `salvage` command
  has been added to the `bixcoin-wallet` tool which performs the salvage
...
`bixcoin.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
Bitcoin Core will now just log warnings instead of creating new wallets with
new keys and addresses like previous releases did.

New wallets can be created through the GUI (which has a more prominent create
wallet option), through the `bixcoin-cli createwallet` or `bixcoin-wallet
...
- A new `fee_rate` parameter/option denominated in satoshis per vbyte (sat/vB)
  ...
  `walletcreatefundedpsbt` still exists for setting a fee rate in BIX per 1,000
  vbytes (BIX/kvB), but it is expected to be deprecated soon to avoid
  confusion. For these RPCs, the fee rate error message is updated from BIX/kB
  to sat/vB and the help documentation in BIX/kB is updated to BIX/kvB. The
  `send` and `sendtoaddress` RPC examples are updated to aid users in creating
  transactions with explicit fee rates. (#20305, #11413)

- The `bumpfee` RPC `fee_rate` option is changed from BIX/kvB to sat/vB and the
  ...
  difference between BIX/kvB and sat/vB units means that a transaction with a
  fee rate mistakenly calculated in BIX/kvB rather than sat/vB should raise an
...
- Wallets created or loaded in the GUI will now be automatically loaded on
  ...
  `\<datadir\>/settings.json` and augments any command line or `bixcoin.conf`
...
- #20198 Show name, format and if uses descriptors in bixcoin-wallet tool (jonasschnelli)
...
- #19133 rpc, cli, test: add bixcoin-cli -generate command (jonatack)
...
- #20055 Set HTTP Content-Type in bixcoin-cli (laanwj)
...
### Build system
- #18504 Drop bixcoin-tx and bixcoin-wallet dependencies on libevent (ryanofsky)
...
- #18653 add coverage for bixcoin-cli -rpcwait (jonatack)
...
- #18817 Document differences in bixcoind and bixcoin-qt locale handling (practicalswift)
