...
We recommend that everybody running prior versions of bixcoind/Bixcoin-Qt
...
in your terminal, then install the bixcoin-qt package:
  sudo apt-get install bixcoin-qt
...
* (Windows only): enable ASLR and DEP for bixcoin-qt.exe
* (Windows only): add meta-data to bixcoin-qt.exe (e.g. description)
...
* Bash programmable completion for bixcoind(1)
