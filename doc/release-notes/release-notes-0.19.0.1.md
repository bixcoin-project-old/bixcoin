...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `bixcoind -help` output for these two options for
...
- The `testnet` field in `bixcoin-cli -getinfo` has been renamed to
...
- If a Tor hidden service is being used, Bitcoin Core will be bound to
  the standard port 9424 even if a different port is configured for
...
- When creating a transaction with a fee above `-maxtxfee` (default 0.1
  BIX), the RPC commands `walletcreatefundedpsbt` and
  `fundrawtransaction` will now fail instead of rounding down the fee.
  Be aware that the `feeRate` argument is specified in BIX per 1,000
...
- #15639 bixcoin-wallet tool: Drop libbitcoin_server.a dependency (ryanofsky)
...
- #16521 Use the default maxfeerate value as BIX/kB (Remagpie)
...
- #16366 init: Use InitError for all errors in bixcoind/qt (MarcoFalke)
...
- #16258 [MSVC]: Create the config.ini as part of bixcoind build (NicolasDorier)
...
- #15352 Reduce noise level in test_bixcoin output (practicalswift)
...
- #16277 Suppress output in test_bixcoin for expected errors (gertjaap)
...
- #16929 follow-up to rpc: default maxfeerate value as BIX/kB (jonatack)
...
- #16489 log: harmonize bixcoind logging (jonatack)
...
- #15513 add "sections" info to example bixcoin.conf (fanquake)
...
- #16448 add note on precedence of options in bixcoin.conf (fanquake)
...
- #16621 add default bixcoin.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the bixcoin-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, bixcoin.conf, and Info.plist.in (ch4ot1c)
