...
- `bixcoind` and `bixcoin-qt` will now raise an error on startup
 if a datadir that is being used contains a bixcoin.conf file that
 will be ignored, which can happen when a datadir= line is used in
 a bixcoin.conf file. The error message is just a diagnostic intended
 ...
 the bixcoin.conf contained in it. (#27302)
...
```sh
src/bixcoin-cli -named bumpfee txid fee_rate=100
```

instead of

```sh
src/bixcoin-cli -named bumpfee txid options='{"fee_rate": 100}'
```
