...
installer (on Windows) or just copy over `/Applications/Bixcoin-Qt` (on Mac)
...
- `-includeconf=<file>` can be used to include additional configuration files.
  Only works inside the `bixcoin.conf` file, not inside included files or from
  ...
  as bixcoin.conf will still include `relative.conf`.
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the bixcoin.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
be used in V0.17 by starting bixcoind with the '-deprecatedrpc=accounts'
...
- New `fees` field introduced in `getrawmempool`, `getmempoolancestors`, `getmempooldescendants` and
   ...
   and `descendant` denominated in BIX. This new field deprecates previous fee fields, such as
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart bixcoind with
...
- When running bixcoind with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If bixcoind
...
- #12240 `6f8b345` Introduced a new `fees` structure that aggregates all sub-field fee types denominated in BIX (mryandao)
...
- #13604 `75bea05` Add depends 32-bit arm support for bixcoin-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling bixcoin-qt (TheCharlatan)
...
- #12443 `07090c5` Move common args to bixcoin.conf (MarcoFalke)
...
- #12882 `d96bdd7` Make `test_bixcoin` pass under ThreadSanitzer (clang). Fix lock-order-inversion (potential deadlock) (practicalswift)
...
- #12102 `7fb8fb4` Apply hardening measures in bixcoind systemd service file (Flowdalic)
- #12652 `55f490a` bixcoin-cli: Provide a better error message when bixcoind is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to bixcoind in debian package (ghost)
...
- #13603 `453ae5e` bixcoin-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix bixcoin-cli --version (Empact)
...
- #12877 `2b54155` Use bixcoind in Tor documentation (knoxcard)
...
- #13905 `73a09b4` fixed bixcoin-cli -help output for help2man (hebasto)
