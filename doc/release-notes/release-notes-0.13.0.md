...
- Adding `dbcache=100` in bixcoin.conf
...
For example:
    ...
    $ src/bixcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
The two phases are distinct in the Bixcoin-Qt GUI. During the first one,
...
- #7550 `8b958ab` Input-from-stdin mode for bixcoin-cli (laanwj)
...
- #8153 `75ec320` `fundrawtransaction` feeRate: Use BIX/kB (MarcoFalke)
...
- #7613 `3798e5d` Add autocomplete to bixcoin-qt's console window (GamerSg)
...
- #8256 `af2421c` BUG: bixcoin-qt crash (fsb4000)
- #8257 `ff03c50` Do not ask a UI question from bixcoind (sipa)
...
- #8047 `37f9a1f` Test_framework: Set wait-timeout for bixcoind procs (MarcoFalke)
...
- #7744 `d8e862a` test_framework: detect failure of bixcoind startup (laanwj)
