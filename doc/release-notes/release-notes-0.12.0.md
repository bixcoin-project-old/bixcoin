...
This allows running bixcoind without having to do any manual configuration.
...
`bixcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /bitcoinrpc>
        ...
        # optional bypass bixcoind rpc basic auth
...
Restarting bixcoind will no longer clear out the list of banned peers; instead
...
- #6214 `87406aa` [OSX] revert renaming of Bixcoin-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
...
- #6600 `ef4945f` Include bixcoin-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix bixcoin-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of bixcoin-cli on secp256k1 (Wladimir J. van der Laan)
...
- #6387 `11576a5` [bixcoin-cli] improve error output (Jonas Schnelli)
...
- #6504 `b6fee6b` Rationalize currency unit to "BIX" (Ross Nicoll)
