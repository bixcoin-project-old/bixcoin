...
configuration options and toggled while bixcoind is running with the `logging`
RPC.  For instance, launching bixcoind with `-debug` or `-debug=1` will turn on
...
with "play bixcoins" on a test network.
...
i.e. `CPPFLAGS="-DDEBUG_LOCKCONTENTION"`, then build and run bixcoind.

You can then use the `-debug=lock` configuration option at bixcoind startup or
`bixcoin-cli logging '["lock"]'` at runtime to turn on lock contention logging.
It can be toggled off again with `bixcoin-cli logging [] '["lock"]'`.
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_bixcoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_bixcoin --log_level=test_suite
$ valgrind -v --leak-check=full src/bixcoind -printtoconsole
...
# A coverage report will now be accessible at `./test_bixcoin.coverage/index.html`,
...
To profile a running bixcoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep bixcoind` -- sleep 60
```
...
- [Main thread (`bixcoind`)](https://doxygen.bitcoincore.org/bitcoind_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
...
- Net threads:
  ...
  - [ThreadSocketHandler (`b-net`)](https://doxygen.bitcoincore.org/class_c_connman.html#a765597cbfe99c083d8fa3d61bb464e34)
    : Sends/Receives data from peers on port 9424.
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `bixcoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `bixcoin-cli` and the GUI debug console use this table to determine how to
