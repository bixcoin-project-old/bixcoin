...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def parse_args(self):
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use bixcoin-cli instead of RPC for all commands")
    ...
    def set_binary_paths(self):
        ...
        binaries = {
            "bixcoind": ("bixcoind", "BITCOIND"),
            "bixcoin-cli": ("bitcoincli", "BITCOINCLI"),
            "bixcoin-util": ("bitcoinutil", "BITCOINUTIL"),
            "bixcoin-wallet": ("bitcoinwallet", "BITCOINWALLET"),
        }
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'bixcoind', self.options.bixcoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'bixcoin-cli', self.options.bitcoincli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a bixcoind test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple bixcoind test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the bixcoind
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as bixcoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    bixcoind=self.options.bixcoind,
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in bixcoin.conf
    ...
    def skip_if_no_bitcoind_tracepoints(self):
        """Skip the running test if bixcoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("bixcoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_bitcoind_zmq(self):
        """Skip the running test if bixcoind has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("bixcoind has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if bixcoin-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("bixcoin-wallet has not been compiled")

    def skip_if_no_bitcoin_util(self):
        """Skip the running test if bixcoin-util has not been compiled."""
        if not self.is_bitcoin_util_compiled():
            raise SkipTest("bixcoin-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if bixcoin-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("bixcoin-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether bixcoin-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether bixcoin-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_bitcoin_util_compiled(self):
        """Checks whether bixcoin-util was compiled."""
