...
- Set the `self.setup_clean_chain` variable in `set_test_params()` to `True` to
  ...
  mature block subsidies (25x50=1250 BIX) in its wallet. Using them is much more
...
with the bixcoind(s) being tested in a callback-based event loop; the other
implements the test logic.

- `P2PConnection` is the class used to connect to a bixcoind.  `P2PInterface`
...
Test objects for interacting with a bixcoind node over the p2p interface.
