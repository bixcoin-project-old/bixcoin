...
To build Bitcoin Core without Qt, unload or disable the `bixcoin-qt`, `libbitcoin_qt` and `test_bixcoin-qt` projects.
...
To check if `bixcoind` has `RandomizedBaseAddress` enabled or disabled run

```
.\dumpbin.exe /headers src/bixcoind.exe
```
